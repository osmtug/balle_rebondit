...
while running:
    ...
    pygame.draw.circle(screen, (r,v,b), player_pos, taille+1)
    
    pygame.draw.circle(screen, "black", player_pos, taille)
    ...
    if taille+1 + (295-taille*2)<math.sqrt((player_pos.x - screen.get_width() / 2)**2 + (player_pos.y - screen.get_height() / 2)**2):
        ...
        if player_pos.y>screen.get_height() / 2 :
            if player_pos.x> screen.get_width() / 2 and velocitex+tempy>0:
            ...
            if player_pos.x> screen.get_width() / 2 and ok:
                velocitex = -(-velocitex * pourcentx + tempy * (abs(pourcentx)-1))
            elif player_pos.x<= screen.get_width() / 2 and velocitex+tempy<0:
                velocitex = -(-velocitex * pourcentx - tempy * (abs(pourcentx)-1))

        if player_pos.y>screen.get_height() / 2 and player_pos.x> screen.get_width() / 2 and tempx+tempy>0 or player_pos.x<= screen.get_width() / 2 and tempx+tempy<0 and player_pos.y<=screen.get_height() / 2 or ok:
